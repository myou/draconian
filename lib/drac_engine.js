...
var errorHandlingStrategies = [];
errorHandlingStrategies.push(function(paramsRulesErrorList) {
...
DracEngine.draconize = function(paramsRules, func) {
  ...
  return function() {
    paramsRulesErrorList = applyParamsRules(arguments[0], paramsRules);
    errorHandlingStrategies.forEach(function(strategy) {
      strategy(paramsRulesErrorList);
    });
...
applyParamsRules = function(args, paramsRules) {
  ...
  var index = 0;
  for (var paramName in paramsRules) {
    // support both named and non-named parameters
    var paramValue = args[paramName] || args[index];
    var paramRules = paramsRules[paramName];
    index++;
