...
RuleBook.match = function(paramValue, ruleValue) {
  if (typeof paramValue !== 'string') {
    return 'Invalid parameter, must be string to try to match';
  }
...
RuleBook.notEmpty = function(paramValue, ruleValue) {
  if (typeof paramValue !== 'string') {
    return 'Invalid parameter, nonempty parameter must be string';
  }
...
RuleBook.isIn = function(paramValue, ruleValue) {
  if (ruleValue.indexOf(paramValue) === -1) {
    return 'Invalid parameter, not one of predefined values';
  }
};
...
RuleBook.notIn = function(paramValue, ruleValue) {
  if (ruleValue.indexOf(paramValue) !== -1) {
    return 'Invalid parameter, matched one of predefined forbidden values';
  }
};
...
RuleBook.len = function(paramValue, ruleValue) {
  ...
  if (length === undefined) {
    return 'Invalid parameter, does not have length attribute';
  }
  if (length <= ruleValue[0] || length >= ruleValue[1]) {
    return 'Invalid parameter, length outside allowed range';
  }
};
